...
cc.Class({
	...
	onEnable: function () {
		cc.RedT.send({mini_poker: {get_data:true}});
		this.get_data();
	},
	onDisable: function () {
		this.reset();
	...
	get_data: function(page = 1){
		cc.RedT.send({mini_poker:{get_top:{sort: this.inputSort, page: page}}});
	},
	onClickSetName: function(e, bet) {
		...
		if (name.length < 3) {
			...
			cc.RedT.send({mini_poker: {name_hu:{name: name, bet: bet}}});
		}
	},
	onData: function(data){
		...
		if (!!data.eventData) {
            cc.RedT.dialog.EventMiniPoker.onData(data.eventData);
        }
	},
