...
cc.Class({
    ...
    onEnable: function () {
        cc.RedT.send({candy: {get_data:true}});
        this.get_data();
    },
    onDisable: function () {
        this.reset();
    ...
    get_data: function(page = 1){
        cc.RedT.send({candy:{get_top:{sort: this.inputSort, page: page}}});
    },
    onClickSetName: function(e, bet) {
        ...
        if (name.length < 3) {
            ...
            cc.RedT.send({candy: {name_hu:{name: name, bet: bet}}});
        }
    },
    ...
    doiCheDo: function(e, status = null){
        !!status && cc.RedT.send({candy:{chedo: e.target.name}});
    },
